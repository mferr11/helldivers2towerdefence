...
public class RadiusDisplayComponent extends RenderComponent {  // Changed from Component
    ...
    public void create() {
        ...
        if (isPreview) {
            ...
            ServiceLocator.getGameAreaEvents().addListener("updateTowerPreview", 
                (EventListener1<TowerType>) this::updateRadius);
        } else {
            ServiceLocator.getGameAreaEvents().addListener("towerClicked", 
                (EventListener1<Entity>) this::handleTowerClick);
            ServiceLocator.getGameAreaEvents().addListener("deselectTower", 
            () -> isSelected = false);
        }
    }
